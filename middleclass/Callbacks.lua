...
local function _changeClassDict(theClass)
  ...
  --print("			instanceDict.__index 01 ",instanceDict.__index)
  instanceDict.__index = function(instance, methodName)

	--print("Index in callbacks " , instance , methodName)
	...
	rawset(theClass, "__callbacksDict" , instanceDict)
	--print("			instanceDict.__index 02 ",instanceDict.__index)
...
local function _addCallback( theClass, beforeOrAfter, methodName, callback, ...)
	--print("Adding callback ",methodName)
...
local function _removeCallbacks( theClass, beforeOrAfter, methodName)
	--print("Remove callback ",methodName)

	assert(_hasEntry(theClass, methodName) == true, 'class ' .. theClass.name .. " not have callbacks")
	

	local entry = _getEntry(theClass, methodName)
	if (beforeOrAfter == "before") then
		entry.before = {}
	elseif (beforeOrAfter == "after") then
		entry.after = {}
	else
	end
...
function Callbacks:included(theClass)
  ...
  if includes(Callbacks, theClass) then return end
	...
	print(" ### include Callbacks in ",theClass)
...
function Callbacks.before(theClass, methodName, callback, ...)
	if (callback) then
		...
		_removeCallbacks(theClass, 'before', methodName)
	end
end
...
function Callbacks.after(theClass, methodName, callback, ...)
	if (callback) then
		...
		_removeCallbacks(theClass, 'after', methodName)
	end
end
